...
public class ConnectThreadScan extends Thread {
    ...
    public boolean connect(BluetoothDevice bTDevice, Context mContext, Callable callable) {
        ...
        try {if (ActivityCompat.checkSelfPermission(mContext, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(mContext, android.Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            //Toast.makeText(mContext.this, "First enable LOCATION ACCESS in settings.", Toast.LENGTH_LONG).show();
        }
